...
namespace XamariNES.CPU.Tests
{
    ...
    public class CPX_Tests
    {
        ...
        {
            var mapper = new NROM(new byte[] { 0xE0, 0x7F }, null);
            var cpu = new Core(mapper) { X = 0x7F };
        ...
        {
            var mapper = new NROM(new byte[] { 0xE0, 0x01 }, null);
        ...
        {
            var mapper = new NROM(new byte[] { 0xE4, 0x00 }, null);
            var cpu = new Core(mapper) { X = 0x7F };
        ...
        {
            var mapper = new NROM(new byte[] { 0xEC, 0x00, 0x00 }, null);
            var cpu = new Core(mapper) { X = 0x7F };
            cpu.CPUMemory.WriteByte(0x00, 0x7E);
